...
def parse_agent_key(agent_type: str, description: str | None = None) -> str:
    """
...
class ModelConfig:
    ...
    @classmethod
    def from_config(cls, model_config: str | None = None) -> "ModelConfig":
        ...
        agents_config = get_application_config().agents
        ...
        max_tokens = agents_config.get("max_tokens", DEFAULT_MAX_TOKENS)
        temperature = agents_config.get("temperature", DEFAULT_TEMPERATURE)
        ...
        match model_type:
            ...
            case ModelType.BEDROCK:
                raise NotImplementedError("Bedrock model is not supported yet (need AWS specific configurations)")
                return cls(
                    model_type=model_type,
...
def get_or_create_ai_model(model_config: ModelConfig | None) -> Model:
    ...
    if model_config.base_url:  # type: ignore
